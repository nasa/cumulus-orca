...
for taskdir in `ls -d tasks/* | egrep -v 'request_status_for|shared_libraries|db_deploy|package|post_copy_request_to_queue' `
do
...
for task in $(ls -d tasks/* | egrep "request_status_|db_deploy|post_copy_request_to_queue")
do
  echo
