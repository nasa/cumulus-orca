...
for taskdir in $(ls -d tasks/* | egrep "copy_to_glacier$|dr_dbutils|pg_utils")
do
...
for task in $(ls -d tasks/* | egrep -v "copy_to_glacier$|dr_dbutils|pg_utils|shared_libraries|package")
do
  echo
