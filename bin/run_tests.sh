...
for taskdir in `ls -d tasks/* | egrep -v 'request_status_for|shared_libraries|db_deploy|package|post_to_database|copy_files_to_archive|post_copy_request_to_queue|orca_catalog_reporting_dummy|request_files' `
do
...
for task in $(ls -d tasks/* | egrep "request_status_|db_deploy|post_to_database|copy_files_to_archive|post_copy_request_to_queue|orca_catalog_reporting_dummy|request_files")
do
  echo
