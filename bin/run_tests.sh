...
for taskdir in `ls -d tasks/* | egrep -v 'request_status_for|shared_libraries|copy_files_to_archive' `
do
...
for task in $(ls -d tasks/* | egrep 'request_status_|copy_files_to_archive')
do
  echo
