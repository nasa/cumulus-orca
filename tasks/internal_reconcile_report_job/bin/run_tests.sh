...
isort \
    ...
    --force-grid-wrap 0 \
    -m 3 \
    internal_reconcile_report_job.py test

echo "INFO: Formatting with black ..."
black internal_reconcile_report_job.py test


## Run code smell and security tests using bandit
echo "INFO: Running code smell security tests ..."
bandit -r internal_reconcile_report_job.py test
let return_code=$?
...
coverage run --source internal_reconcile_report_job -m pytest
let return_code=$?
