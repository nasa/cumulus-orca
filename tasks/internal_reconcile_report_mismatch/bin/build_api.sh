...
## DESCRIPTION
## -----------------------------------------------------------------------------
## Builds the internal_reconcile_report_mismatch API documentation.
##
##
## USAGE
## -----------------------------------------------------------------------------
...
BASEDIR=$(dirname $0)
if [ "$BASEDIR" != "bin" ]; then
  >&2 echo "ERROR: This script must be called from the root directory of internal_reconcile_report_mismatch [bin/build.sh]."
  exit 1
fi
...
pydoc-markdown -I . -m internal_reconcile_report_mismatch --render-toc > API.md
let return_code=$?
