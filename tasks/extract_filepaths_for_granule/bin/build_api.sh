...
## DESCRIPTION
## -----------------------------------------------------------------------------
## Builds the extract_filepaths_for_granule API documentation.
##
##
## USAGE
## -----------------------------------------------------------------------------
...
BASEDIR=$(dirname $0)
if [ "$BASEDIR" != "bin" ]; then
  >&2 echo "ERROR: This script must be called from the root directory extract_filepaths_for_granule [bin/build_api.sh]."
  exit 1
fi
...
pydoc-markdown -I . -m extract_filepaths_for_granule --render-toc > API.md
let return_code=$?
