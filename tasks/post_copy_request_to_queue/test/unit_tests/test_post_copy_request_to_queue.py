...
class TestPostCopyRequestToQueue(TestCase):
    ...
    @patch("post_copy_request_to_queue.task")
    def test_handler_multiple_s3_records_raises_error(
        self,
        mock_task: MagicMock,
    ):
        """
        Code can currently only handle one record in s3 event.
        """
        key_path_1 = f"{uuid.uuid4()}.ext"
        bucket_name_1 = f"{uuid.uuid4()}-bucket"
        key_path_2 = f"{uuid.uuid4()}.ext"
        bucket_name_2 = f"{uuid.uuid4()}-bucket"
