...
## DESCRIPTION
## -----------------------------------------------------------------------------
## Builds the post_copy_request_to_queue API documentation.
##
##
## USAGE
## -----------------------------------------------------------------------------
...
BASEDIR=$(dirname $0)
if [ "$BASEDIR" != "bin" ]; then
  >&2 echo "ERROR: This script must be called from the root directory post_copy_request_to_queue [bin/build_api.sh]."
  exit 1
fi
...
pydoc-markdown -I . -m post_copy_request_to_queue --render-toc > API.md
let return_code=$?
