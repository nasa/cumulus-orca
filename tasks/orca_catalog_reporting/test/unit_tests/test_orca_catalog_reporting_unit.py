...
class TestOrcaCatalogReportingUnit(
    ...
    @patch("orca_shared.database.shared_db.get_configuration")
    @patch("cumulus_logger.CumulusLogger.setMetadata")
    def test_handler_missing_properties_uses_default(self,
                                                     mock_setMetadata: MagicMock,
                                                     mock_get_configuration: MagicMock,
                                                     mock_task: MagicMock):
        """
    ...
    @patch("orca_shared.database.shared_db.get_configuration")
    @patch("cumulus_logger.CumulusLogger.setMetadata")
    def test_handler_missing_page_index_returns_error(self,
                                                      mock_setMetadata: MagicMock,
                                                      mock_get_configuration: MagicMock,
    ...
    @patch("orca_shared.database.shared_db.get_configuration")
    @patch("cumulus_logger.CumulusLogger.setMetadata")
    def test_handler_missing_end_timestamp_returns_error(self,
                                                         mock_setMetadata: MagicMock,
                                                         mock_get_configuration: MagicMock,
                                                         mock_create_http_error_dict: MagicMock):
        """
    ...
    def test_task_happy_path(self,
        ...
        result = orca_catalog_reporting.task(provider_id, collection_id, granule_id, start_timestamp, end_timestamp,
                                             page_index,
                                             db_connect_info)

        mock_get_user_connection.assert_called_once_with(db_connect_info)
        mock_query_db.assert_called_once_with(mock_get_user_connection.return_value, provider_id, collection_id,
                                              granule_id, start_timestamp, end_timestamp, page_index)
        self.assertEqual({"anotherPage": False, "granules": granules}, result)

    @patch("orca_catalog_reporting.query_db")
    @patch("orca_shared.database.shared_db.get_user_connection")
    def test_task_another_page(self,
                               mock_get_user_connection: MagicMock,
                               mock_query_db: MagicMock):
        """
        Task should call query_db, and send granules back.
        ...
        mock_query_db.assert_called_once_with(mock_get_user_connection.return_value, provider_id, collection_id,
                                              granule_id, start_timestamp, end_timestamp, page_index)
        self.assertEqual({"anotherPage": True, "granules": granules[0:orca_catalog_reporting.PAGE_SIZE]}, result)

    @patch("orca_catalog_reporting.get_catalog_sql")
    def test_query_db_happy_path(self,
                                 mock_get_catalog_sql: MagicMock):
        """
        ...
        result = orca_catalog_reporting.query_db(mock_engine, provider_id, collection_id, granule_id, start_timestamp,
                                                 end_timestamp, page_index)

        mock_enter.__enter__.assert_called_once_with()
        mock_execute.assert_called_once_with(mock_get_catalog_sql.return_value, [{
