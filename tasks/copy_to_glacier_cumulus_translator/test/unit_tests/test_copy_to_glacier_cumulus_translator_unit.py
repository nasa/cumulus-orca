...
class TestCopyToGlacierCumulusTranslatorUnit(
    ...
    def test_handler_happy_path(
        self, mock_setMetadata: MagicMock, mock_run_cumulus_task: MagicMock
    ):
        """
    ...
    def test_handler_passthrough(
        ...
        event = {"task_config": expected_config, "payload": expected_input}

        context = Mock()

        mock_task.return_value = {
            "granules": [{"granuleId": "some_granule_id", "files": []}]
        }

        result = copy_to_glacier_cumulus_translator.handler(event, context)
        self.assertEqual(mock_task.return_value, result["payload"])
        mock_task.assert_called_once_with(
            {"input": expected_input, "config": expected_config}, context
        )

    # noinspection PyPep8Naming
    @patch("cumulus_logger.CumulusLogger.setMetadata")
    def test_handler_and_task_back_and_forth(self, mock_setMetadata: MagicMock):
        """
    ...
    def test_task_happy_path(self):
        ...
        event = {"config": expected_config, "input": expected_input}
        context = Mock()
