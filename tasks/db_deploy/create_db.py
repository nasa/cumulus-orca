...
def create_fresh_orca_install(config: Dict[str, str]) -> None:
    ...
    with admin_app_connection.connect() as conn:
        ...
        create_app_schema_role_users(conn, config["user_username"], config["user_password"], config["user_database"], config['admin_username'])
...
def create_app_schema_role_users(connection: Connection, app_username: str, app_password: str, db_name: str, db_admin_name: str) -> None:
    """
    ...
    connection.execute(dbo_role_sql(db_name, db_admin_name))
