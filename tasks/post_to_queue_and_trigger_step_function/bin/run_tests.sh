...
isort \
    ...
    --force-grid-wrap 0 \
    -m 3 \
    post_to_queue_and_trigger_step_function.py test

echo "INFO: Formatting with black ..."
black post_to_queue_and_trigger_step_function.py test
...
coverage run --source post_to_queue_and_trigger_step_function -m pytest
let return_code=$?
