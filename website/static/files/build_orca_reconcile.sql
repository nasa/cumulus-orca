...
BEGIN
  ...
  ALTER TABLE reconcile_status
    ADD CONSTRAINT UNIQUE_reconcile_status
    UNIQUE (value);
  ...
  COMMENT ON TABLE reconcile_jobs IS 'Manages internal reconciliation job information.';
  COMMENT ON COLUMN reconcile_jobs.id IS 'Job ID unique to each internal reconciliation job.';
  COMMENT ON COLUMN reconcile_jobs.orca_archive_location IS 'ORCA S3 Glacier bucket the file resides in.';
  COMMENT ON COLUMN reconcile_jobs.status_id IS 'Current status of the job.';
  COMMENT ON COLUMN reconcile_jobs.inventory_creation_time IS 'Time from the s3 inventory manifest ';
  COMMENT ON COLUMN reconcile_jobs.start_time IS 'Date and time the internal reconcile job started.';
  COMMENT ON COLUMN reconcile_jobs.last_update IS 'Date and time the job status was last updated.';
  COMMENT ON COLUMN reconcile_jobs.end_time IS 'Time the job completed and wrote the report information.';
  COMMENT ON COLUMN reconcile_jobs.error_message IS 'Critical error the job ran into that prevented it from finishing.';


  CREATE TABLE reconcile_s3_objects  ( 
    ...
    ) PARTITION BY RANGE (orca_archive_location);
  COMMENT ON TABLE reconcile_s3_objects IS 'Temporary table that holds the listing from the ORCA S3 bucket to use for comparisons against the ORCA catalog.
  Partitions should be placed around orca_archive_location, followed by indexing on key_path once data is ingested.';
  ...
  COMMENT ON COLUMN reconcile_s3_objects.key_path IS 'Full path and file name of the object in the S3 bucket from the S3 list command.';
  ...
  CREATE TABLE reconcile_catalog_mismatch_report  ( 
    job_id                  	int8 NOT NULL,
  ...
  CREATE TABLE reconcile_orphan_report  ( 
    job_id       	int8 NOT NULL,
    key_path     	text NOT NULL,
    etag         	text NOT NULL,
    last_update  	timestamp with time zone NOT NULL,
  ...
  COMMENT ON COLUMN reconcile_orphan_report.size_in_bytes IS 'AWS size of the object in bytes. Value is obtained from the reconcile_s3_objects (size) column.';
