...
BEGIN
  ...
  COMMENT ON COLUMN reconcile_jobs.orca_archive_location IS 'ORCA S3 Glacier bucket the reconciliation targets.';
  COMMENT ON COLUMN reconcile_jobs.status_id IS 'Current status of the job.';
  COMMENT ON COLUMN reconcile_jobs.inventory_creation_time IS 'Inventory report creation time from the s3 manifest.';
  ...
  COMMENT ON COLUMN reconcile_orphan_report.storage_class IS 'AWS storage class the object is in. Value is obtained from the reconcile_s3_objects (storage_class) column.';
