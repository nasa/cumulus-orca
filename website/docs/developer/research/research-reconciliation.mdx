...
As a general breakdown of the flow, the Lambda or Workflow will need to do the
following:
...
| orca_archive_location   | `string`     | ORCA S3 Glacier bucket the reconciliation targets.   | Location in S3. |
| status_id               | `int8`       | Current status of the job.                           | Generally reflect the stages of reconciliation. |
| inventory_creation_time | `timestamptz`| Inventory report creation time from the s3 manifest. | Cannot be NULL  |
