...
Additionaly, results should be ordered by granule_id.
...
- [AWS API Upload Part Copy Reference](https://docs.aws.amazon.com/AmazonS3/latest/API/API_UploadPartCopy.html)
...
The following provides an architecture of how ORCA internal reconciliation
...
#### Internal Reconciliation Jobs Table reconcile_jobs
...
| orca_archive_location   | `string`     | ORCA S3 Glacier bucket the file resides in. | Location in S3. |
| status_id               | `int8`       | Current status of the job.                  | Generally reflect the stages of reconciliation. |  TODO: Switch to int/string pairs
| inventory_creation_time | `timestampz` | Time from the s3 inventory manifest         | Cannot be NULL  |
...
#### S3 Inventory Temporary Table reconcile_s3_objects
...
| delete_marker         | `boolean` | Set to `True` if object is a delete marker. | Items with a value of `True` should be filtered out before generating reports. |
...
#### Internal Reconciliation Reporting Table S3 Missing reconcile_zombie_report
...
- The user is logged into the orca database as the postgres user to run the script.
