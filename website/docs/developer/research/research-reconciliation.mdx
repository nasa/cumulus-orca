...
| -------------- | --------- | ---------------------------------------- | --------------- |
| providerId     | `string`  | providerId that is provided by Cumulus.  | Must be unique. |
...
| Attribute Name | Data Type | Description                               | Notable Items   |
| -------------- | --------- | ----------------------------------------- | --------------- |
| collectionId   | `string`  | collectionId that is provided by Cumulus. | Must be unique per providerId. |
...
| collectionId        | `string`    | Collection ID from Cumulus that refrences the Collections table.            | None                                     |
| cumulusCreateTime   | `timestamp` | Date and time data was originally ingested into cumulus                     | None                                     |
| cumulusGranuleId    | `string`    | granuleId that is provided by Cumulus.                                      | Must be unique per collectionId          |
| executionId         | `string`    | Step function execution ID from AWS.                                        | Unique ID automatically generated by AWS |
| ingestTime          | `timestamp` | Date and time in UTC that the data was originally ingested into ORCA        | None                                     |
| lastUpdate          | `timestamp` | Date and time in UTC that information was updated.                          | None                                     |

Note that the combination of the Cumulus Granule ID (cumulusGranuleId) and
the Cumulus Collection ID (collectionId) is unique in the
...
| granuleId                | `string`  | Granule that the file belongs to refrences the internal ORCA granule ID.   | None                                                |
| name                     | `string`  | Name of the file. (MOD14A1.061.2020245.hdf)                                | Must be unique per granuleId                        |
| orcaArchiveLocation      | `string`  | ORCA S3 Glacier bucket the file resides in.                                | None                                                |
| cumulusArchiveLocation   | `string`  | Cumulus S3 bucket the file is expected to reside in.                       | None                                                |
| keyPath                  | `string`  | Full S3 path to the file including the file name.                          | Must be unique per archive location                 | 
| ingestTime               | `timestamp` | Date and time in UTC that the data was originally ingested into ORCA     | None                                                |
...
| sizeInBytes              | `int`     | Size in bytes of the file. From Cumulus ingest.                            | Part of object passed to archive from Cumulus.      |
| hash                     | `string`  | Checksum hash of the file provided by Cumulus.                             | Part of object passed to archive from Cumulus.      |
| hashType                 | `string`  | Hash type used to calculate the hash value of the file.                    | Part of object passed to archive from Cumulus.      |

Note that in Cumulus the combination of the bucket (cumulusArchiveLocation)
...
| -------------- | --------- | --------------------------------------------------------- | --------------- |
| version        | `string`  | AWS provided version of the file.                         | Must be unique per file name/key_path |
| isLatest       | `boolean` | AWS provided flag denoting that it is the latest version. | Only one can be latest per file name/key_path. Depending of final table structure this may be optional. |
...
| cumulusEtag    | `string`  | Cumulus provided AWS etag of the file in Cumulus.         | Potentially optional. Would need to determine if this information passed along. May be helpful for data management comparisons. |
| sizeInBytes    | `bigint`  | Size in bytes of the file. From Cumulus ingest.           | Part of object passed to archive from Cumulus. |
| hash           | `string`  | Checksum hash of the file provided by Cumulus.            | Part of object passed to archive from Cumulus. |
| hashType       | `string`  | Hash type used to calculate the hash value of the file.   | Part of object passed to archive from Cumulus. |
| ingestTime     | `timestamp` | Date and time in UTC that the file was ingested into ORCA | None |
            ...
            "files": [
              {
                ...
                "cumulusArchiveLocation": "s3://cumulus-archive/MOD14A1/061/...",
