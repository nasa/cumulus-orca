...
It is recommended that we use a "path" parameter that filters based on the file's 'Key'.
...
items need to be re-ingested into ORCA. Note that entries with a last_update time
...
what is expected. Note that any entries with an orca_last_update after the job's
...
| --------------------- | --------- | ----------------------------------------- | --------------- |
| job_id                | `int8`    | Reconcile job the report is a part of.    | FK constraint back to the jobs table with a many to one relationship. |
| orca_archive_location | `string`  | ORCA S3 Glacier bucket the file resides in. | From Catalog. |
