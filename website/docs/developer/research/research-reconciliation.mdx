...
<MyImage
    imageSource={useBaseUrl('img/Research-Reconciliation-orca-inventory-schema-updated.png')}
...
| cumulus_createdAt   | `timestamp` | time filter for Cumulus reconciliation.                              | None                                     |
...
| multipart_chunksize_mb   | `string`  | The maximum size of chunks to use when copying.                            | None                                                |
| ingest_time              | `timestamp` | Date and time in UTC that the data was originally ingested into ORCA     | None                                                |
...
- What is the proper date to use when receiving a date range from Cumulus for a reconciliation report?
  - After discussion with the Cumulus team, we have decided to use `cumulus_createdAt` timestamp.
- Should granule acquisition date be a part of the metadata?
  -  After discussion with the Cumulus team, we have decided to use `cumulus_createdAt` timestamp.
