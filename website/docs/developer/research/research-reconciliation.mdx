...
| Attribute Name   | Data Type | Description                                                            | Notable Items   |
| ---------------- | --------- | ---------------------------------------------------------------------- | --------------- |
| granule_id       | `string`  | granuleId that is provided by Cumulus.                                 | Must be unique per collectionId |
...
| size           | `bigint`  | Size in bytes of the file. From Cumulus ingest.           | Part of object passed to archive from Cumulus. |
        ...
        "granules": [
          {
            ...
            "lasstUpdate": "2020-01-01T23:00:00+0",
            "files": [
              {
...
metadata needed for a granule's files like `providerId` and `collectionId` and
various file information and send it to a queue. The body sent to the queue
would have enough information for another ORCA lambda to write and/or update the
metadata records for the various objects.
...
get additional information like the version as seen in the sample seen below.
Some things to consider when looking at the options
for getting the additional data include speed (how fast can I get the data), cost
(what is the cost impact of additional gets on the  s3 bucket), and complexity
(where is the best place to put these additional gets, is it overly complex)
...
with a PostgreSQL backed. Both PostgreSQL RDS and Amazon Aurora are viable back end
choices that may be utilized. See a comparison article on the databases
...
| size           | `int8`    | Size key value from the S3 listing.       | Should be able to match the size value in the ORCA catalog *File* metadata object. |
...
| size             | `int8`    | Size key value from the S3 listing.       | |
...
- What is the best back population strategy to use for those running older versions of ORCA?
