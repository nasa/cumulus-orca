...
1. In the new branch's `bamboo.yaml`, change `name` from `ORCA Integrator` to `prototype-latest`. In each `repositories` element, change `orca-develop` to `orca test branch`. In each `plan` element, change `OI`/`ODP` in `key` values to `PL`
:::
